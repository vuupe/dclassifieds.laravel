...
@section('content')
    ...
    <div class="box">
        <div class="box-header with-border">
            <h3 class="box-title">{{ trans('admin_common.All Car Engines') }}</h3>
        </div>
        <!-- /.box-header -->
        ...
        {!! csrf_field() !!}
            ...
            <div class="controls">
                ...
                <a href="{{ url('admin/carengine/edit') }}" class="btn btn-primary btn-sm"><i class="fa fa-file-o"></i> {{ trans('admin_common.New Car Engine') }}</a>
            </div>
                ...
                <table id="list_table" class="table table-bordered table-striped table-hover">
                    <thead>
                        <tr>
                            ...
                            <th>{{ trans('admin_common.Car Engine') }}</th>
                            <th></th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($modelData as $k => $v)
                            <tr>
                                ...
                                <td><a href="{{ url('admin/carengine/edit/' . $v['car_engine_id']) }}"><i class="fa fa-edit"></i> {{ trans('admin_common.Edit') }}</a></td>
                                <td><a href="{{ url('admin/carengine/delete/' . $v['car_engine_id']) }}" class="text-danger need_confirm"><i class="fa fa-trash"></i> {{ trans('admin_common.Delete') }}</a></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>

...
<head>
    <meta charset="UTF-8">
    <title>[CONTROL] Ad #<?=$ad->ad_id?></title>
</head>
<body>
    ...
    <?=$ad->ad_email?><br />
    <?=$ad->ad_ip?><br />
    <a href="<?=url('/viewbyuser/' . $ad->user_id)?>">View all ads from <?=$ad->ad_puslisher_name?></a><br /><br />
    ...
    <br /><br />
    <a href="<?=url('/delete/' . $ad->code);?>">Delete this ad</a>
    <br /><br />
    
    <?if(!$ad->same_ads->isEmpty()){?>
        Same Ads:<br/>
        <?foreach($ad->same_ads as $k => $v){?>
            <a href="<?=url(str_slug($v->ad_title) . '-' . 'ad' . $v->ad_id . '.html');?>"><?=$v->ad_title?></a> | <a href="<?=url('/delete/' . $v->code)?>">Delete this ad #<?=$v->ad_id?></a><br />
        <?}//end of foreach?>
    <?}//end of if?>
    <br /><br />
